...
import { AppBar, Toolbar, Box, Tabs, Tab, Button, IconButton, Drawer, List, ListItem, ListItemText, Divider, Typography } from '@mui/material';
...
const Header: React.FC<HeaderProps> = ({ selectedTab, onTabChange }) => {
  ...
  return (
    <>
      <AppBar position="static" sx={{ backgroundColor: theme.palette.primary.main, width: '100%', boxShadow: '0px 4px 6px rgba(0,0,0,0.1)' }}>
      ...
      <Box sx={{ position: 'fixed', bottom: { xs: '10px', sm: '20px', md: '30px' }, right: { xs: '10px', sm: '10px', md: '10px' }, p: 2, display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 1 }}>
      ...
      <Drawer
        ...
        sx={{
          '& .MuiDrawer-paper': {
            width: 250,
            backgroundColor: theme.palette.primary.main, // Primary color for drawer background
          ...
          <List>
            {tabLabels.map((tab, index) => (
              <ListItem 
                button 
                key={index} 
                onClick={() => { handleTabChange(null as any, tab.route); handleDrawerToggle(); }}
                sx={{
                  backgroundColor: selectedTab === tab.route ? theme.palette.secondary.main : 'transparent',
                  '&:hover': {
                    backgroundColor: theme.palette.secondary.light, // Lighter gold on hover
                  },
                }}
              >
                <ListItemText 
